...
- **Model evaluation:**
  ...
  - **Model and hyperparameter choice:** Meta-models and base learner were chosen via stratified K-fold CV. **T-learner + ElasticNet** was selected: highest mean AUUC (≈0.033, Qini ≈0.32), stable across folds, Qini curve above other models until ~0.5 proportion and above random. **Grid search** tuned the base learner (best: `alpha=0.001`, `l1_ratio=0.5`, `max_iter=5000`; AUUC = 0.0339 ± 0.0158, Qini = 0.328 ± 0.273). The **final model** on the full training set achieved AUUC 0.0456, Qini 0.6120, Uplift@10% 0.0761, Uplift@20% 0.0736.
  ...
  - **Train (80%) vs holdout (20%) evaluation:** On the same 80/20 split used for diagnostic plots, AUUC, Qini, and uplift@k were computed for both partitions. **Train (80%):** AUUC 0.0443, Qini 0.6410, Uplift@5% 0.0938, Uplift@10% 0.0853, Uplift@15% 0.0773, Uplift@20% 0.0747. **Holdout (20%):** AUUC 0.0571, Qini 0.7111, Uplift@5% 0.1619, Uplift@10% 0.1333, Uplift@15% 0.1225, Uplift@20% 0.0980.
  ...
  - **Test set:**
    - **Four-quadrant segments** (Radcliffe-style, from predicted uplift and baseline): **Persuadables** (uplift > ε)—the only segment to target, because they are the ones who gain from treatment; **Sure Things** (|uplift| ≤ ε, low baseline churn)—would retain anyway, so outreach is wasted; **Lost Causes** (|uplift| ≤ ε, high baseline churn)—will churn anyway, so outreach has no gain; **Do-Not-Disturb** (uplift < −ε)—treatment has a negative effect, so they must be avoided. Segmenting this way avoids wasted spend and prevents harming users who react badly.
    - **SHAP:** On the full test set, the most influential features for predicted uplift are `wellco_web_older`, `n_claims`, and `wellco_web_visits_count` (negative—lower activity/claims increase predicted uplift) and `days_since_signup` (positive—longer tenure → more responsive). For the **top 10% by predicted uplift**, SHAP shows uplift is pushed up by low historical claims and web visits and high tenure/recent activity, and pushed down by high claim counts or very new users.
